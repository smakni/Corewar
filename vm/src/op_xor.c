...
/*   Updated: 2019/06/30 18:39:39 by sabri            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/vm.h"

static void	xor_first_param(t_env *env, unsigned int j, t_op_vars *data,
				int *nb_reg)
{
...
static void	xor_second_param(t_env *env, unsigned int j, t_op_vars *data,
				int *nb_reg)
{
...
static void	xor_third_param(t_env *env, unsigned int j, t_op_vars *data,
				int nb_reg[3])
{
...
void		op_xor(t_env *env, unsigned int j)
{
	...
	data.cursor = 1;
	if (j == 177 && env->cycle_index == 23119)
		ft_printf("debug\n");
	if (check_args(env, j, data.cursor, 3))
	{
		nb_reg[0] = 1;
		nb_reg[1] = 1;
		xor_first_param(env, j, &data, &nb_reg[0]);
		xor_second_param(env, j, &data, &nb_reg[1]);
		if (nb_reg[0] >= 1 && nb_reg[0] <= REG_NUMBER
				&& nb_reg[1] >= 1 && nb_reg[1] <= REG_NUMBER)
			data.v1 ^= data.v2;
		data.cursor++;
		...
		xor_third_param(env, j, &data, nb_reg);
		data.cursor++;
	}
	else
