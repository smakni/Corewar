...
/*   Updated: 2019/06/13 18:14:25 by smakni           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		op_sub(t_env *env, unsigned int j)
{
	...
	if (check_args(env, j, &cursor, 3))
	{
		...
		cursor++;
		nb_reg1 = env->champ[j].op.saved[cursor];
		save_param(env, j, nb_reg1, REG_CODE, 0);
		if (nb_reg1 >= 1 && nb_reg1 <= 16)
		{
			reg_content = env->champ[j].r[env->champ[j].op.saved[cursor]];
			diff = reg_content;
		}
		else
			diff = 0;
		cursor++;
		nb_reg2 = env->champ[j].op.saved[cursor];
