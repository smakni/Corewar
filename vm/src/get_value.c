...
int			get_value(t_env *env, unsigned j, int *cursor, int param)
{
	...
	value = 0;
	if (type_param(env->champ[j].op.saved[1], param) == REG_CODE)
	{
		(*cursor)++;
		value = env->champ[j].r[env->champ[j].op.saved[*cursor]];
	}
	else if (type_param(env->champ[j].op.saved[1], param) == DIR_CODE)
	{
		(*cursor)++;
		value = read_bytes(env->champ[j].op.saved, *cursor, DIR_SIZE);
		//save_param(env, j, value, DIR_CODE);
		*cursor += DIR_SIZE - 1;
	}
	else if (type_param(env->champ[j].op.saved[1], param) == IND_CODE)
	{
		(*cursor)++;
		value = read_bytes(env->champ[j].op.saved, *cursor, IND_SIZE);
...
int			get_value_index(t_env *env, unsigned j, int *cursor, int param)
{
	...
	else
	{
		(*cursor)++;
		value = read_bytes(env->champ[j].op.saved, *cursor, IND_SIZE);
		//if (type_param(env->champ[j].op.saved[1], param) == IND_CODE)
			//save_param(env, j, value, IND_CODE);
		//else
			//save_param(env, j, value, DIR_CODE);
		(*cursor) += IND_SIZE - 1;
