...
static void print_pc(t_env *env, unsigned j, int save)
{
	...
	ft_putendl("");
	wattroff(env->infos, COLOR_PAIR(env->proc[j].color));
	wrefresh(env->infos);
}
...
void aff_operations_visu(t_env *env, unsigned j, int save)
{
	...
	wattron(env->infos, COLOR_PAIR(env->proc[j].color));
	...
	{
		if (env->proc[j].op.name != NULL)
			wprintw(env->infos, "%s", env->proc[j].op.name);
		while (env->proc[j].op.param[i])
			wprintw(env->infos, " %s", env->proc[j].op.param[i++]);
		if (env->proc[j].op.saved[0] == 0x01 && (ft_strcmp(env->proc[j].op.param[0], "-1") == 0 || ft_strcmp(env->proc[j].op.param[0], "-2") == 0 || ft_strcmp(env->proc[j].op.param[0], "-3") == 0 || ft_strcmp(env->proc[j].op.param[0], "-4") == 0))
