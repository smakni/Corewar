...
/*   Updated: 2019/06/13 18:07:27 by smakni           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		op_ldi(t_env *env, unsigned int j)
{
	...
	if (check_args(env, j, &cursor, 3))
	{
		...
		{
			if (type_param(env->proc[j].op.saved[1], 1) == IND_CODE)
				sum = read_bytes(env->memory, env->proc[j].pc + (v1 % IDX_MOD), REG_SIZE);
			else
				sum = v1;
			sum += v2;
			sum = (env->proc[j].pc + (sum % (IDX_MOD))) % MEM_SIZE;
			if (sum < 0)
				sum += MEM_SIZE;
